    ...
    <div class="modal fade" tabindex="-1" id="appear">
        <div class="modal-dialog">
            <div class="modal-content">
                    ...
                    <form id="formappear">
                            <div class="form-group col-md-12">
                                ...
                                <select class="mb-10 form-select form-select-solid selDiv" aria-label="Select example" name="appear" id="appear_input">
                                    ...
                                    <option value="best_seller"> {{ __('lang.best_seller') }}</option>
                                    <option value="first_home_page"> {{ __('lang.first_home_page') }}</option>
                                    <option value="most_recent"> {{ __('lang.most_recent') }}</option>
                                    <option value="only_product"> {{ __('lang.only_product') }}</option>
                                </select>
                            </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ __('lang.but_cancel') }}</button>
                    <button class="btn btn-success" id="save-appear">{{ __('lang.but_save') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">{{ __('lang.titleproducts') }}</span>
            </h3>
            @can('Product-Create')
            <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover"
                 title="Click to add a productsegory">

                    <a href="{{ route('products.create', ['category' => $id ?? ""]) }}" class="btn btn-sm btn-light-primary">
        ...
        <div class="card-body py-3">
            ...
            <div class="table-responsive" id="table-data">
                <!--begin::Table-->
                @include('dashboard.Products.table-data')
            </div>
        </div>
    </div>
@endsection
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '#status' ,function (e){
            ...
            $.ajax({
                type: 'post',
                url: '{{ url('admin/products/Stauts') }}/'+id,
                data: '',
                success: function (response) {
                    $.ajax({
                        url: "{{ route('products.index') }}?type="+type
                    }).done(function(data) {
        ...
        function delete_f(id) {
            $(document).off("click", "#save-delete").on("click", "#save-delete", function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: '{{ url('admin/products/destroy') }}/' + id,
                    data: '',
                    success: function (response) {
                        Swal.fire({
        ...
        function appear_update(id) {
            $(document).off("click", "#save-appear").on("click", "#save-appear", function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: '{{ url('admin/products/appearUpdate') }}/' + id,
                    data: formdata,
                    contentType: false,
                    processData: false,
                    success: function (response) {
