    ...
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">{{ __('lang.titleshippingoptions') }}</span>
            </h3>
            <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover"
                 title="Click to add a shipping options">
                <a href="{{ route('shippingoptions.create') }}" class="btn btn-sm btn-light-primary">
                    ...
                    </span> {{ __('lang.addshippingoptions') }}</a>
            </div>
        </div>
        ...
        <div class="card-body py-3">
            ...
            <div class="table-responsive" id="table-data">
                <!--begin::Table-->
                @include('dashboard.Shippingoptions.table-data')
            </div>
        </div>
    </div>
@endsection
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '#status' ,function (e){
            ...
            $.ajax({
                type: 'post',
                url: '{{ url('admin/shippingoptions/Stauts') }}/'+id,
                data: '',
                success: function (response) {
                $.ajax({
                        url: "{{ route('shippingoptions.index') }}"
                    }).done(function(data) {
        ...
        function delete_f(id) {
            $(document).off("click", "#save-delete").on("click", "#save-delete", function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: '{{ url('admin/shippingoptions/destroy') }}/' + id,
                    data: '',
                    success: function (response) {
                        Swal.fire({
