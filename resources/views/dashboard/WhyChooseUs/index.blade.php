    ...
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">{{ __('lang.titlewhychooseus') }}</span>
            </h3>
            @can('Why-Choose-us-Create')
            <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover"
                 title="Click to add  Why Choose Us">
                <a href="{{ route('whychooseus.create') }}" class="btn btn-sm btn-light-primary">
        ...
        <div class="card-body py-3">
            ...
            <div class="table-responsive" id="table-data">
                <!--begin::Table-->
                @include('dashboard.WhyChooseUs.table-data')
            </div>
        </div>
    </div>
@endsection
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '#status' ,function (e){
            ...
            $.ajax({
                type: 'post',
                url: '{{ url('admin/whychooseus/Stauts') }}/'+id,
                data: '',
                success: function (response) {
                    $.ajax({
                        url: "{{ route('whychooseus.index') }}"
                    }).done(function(data) {
        ...
        function delete_f(id) {
            $(document).off("click", "#save-delete").on("click", "#save-delete", function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: '{{ url('admin/whychooseus/destroy') }}/' + id,
                    data: '',
                    success: function (response) {
                        Swal.fire({
