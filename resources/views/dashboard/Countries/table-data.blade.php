<table class="table table-row-bordered gy-5" id="countries">
    <!--begin::Table head-->
    <thead>
...
{{--                                {{ mb_substr($key->name_ar , 0 , 10 , 'utf-8').' ...' }}--}}
            ...
            <td>
                @if($key->status == '1')
                    <button class="btn btn-success" id="status" data-id="{{ $key->id }}">{{ __('lang.active') }}</button>
                @else
                    <button class="btn btn-danger" id="status" data-id="{{ $key->id }}">{{ __('lang.notactive') }}</button>
                @endif
            </td>
            <td>
                @can('City-List')
                <a href="{{ url(\Illuminate\Support\Facades\App::getLocale().'/admin/cities/show') }}/{{ $key->id }}" class="btn btn-icon btn-light-success btn-sm me-1">
                ...
                @endcan
                @can('Country-Edit')
                <a href="{{ url(\Illuminate\Support\Facades\App::getLocale().'/admin/countries/edit') }}/{{ $key->id }}" class="btn btn-icon btn-light-warning btn-sm me-1">
                                    <span class="svg-icon svg-icon-3">
                    ...
                    @endcan
                @can('Country-Delete')
                <a class="btn btn-icon btn-light-danger btn-sm"  data-bs-toggle="modal" data-bs-target="#kt_modal_1" id="delete" data-id="{{ $key->id }}">
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg"
